...
import Graphics.Gloss.Data.Color qualified (white)
import Graphics.Gloss.Data.Point.Arithmetic qualified as Pt
import Graphics.Gloss.Interface.IO.Simulate
  ( Color,
    Display (InWindow),
    Picture (Color, Pictures, ThickCircle, Translate),
    Point,
    greyN,
    makeColor,
    polygon,
...
startGame dirs =
  let cells = [(x - width / 2, y - width / 2) | x <- [0 .. width], y <- [0 .. width]]
   in Maze
    ...
    Color black $
      Pictures $
        let s = 1000 / width
         in map
              ( \(c@(r, g, bl, z), (a, b)) ->
                  if c == rgbaOfColor black
                    then Color black $ Translate (a * s) (b * s) (line [(0, 0), (0, s), (s, s), (s, 0), (0, 0)])
                    else Color (makeColor r g bl z) $ Translate (a * s) (b * s) (polygon [(0, 0), (0, s), (s, s), (s, 0), (0, 0)])
              )
